...
2. PROGRESS.md — retrieval table and current-state section
...
Step 3 — Update PROGRESS.md. Log wins and retrieval scores. Tag [FIRST TIME] on first clean unprompted retrieval.
Overwrite the ## Next session block with exactly three lines:
...
- Next session's contract: first action, retrieval question, carry-forwards — then write the same into TASKS.md Now and PROGRESS.md ## Next session.
...
Step 6 — Closing retrieval check. Ask the same question that opened this session. Score 0/1/2 and record in PROGRESS.md. If score < 2, add to carry-forwards.
...
Confirm all of: TASKS.md updated, PROGRESS.md updated, notes file written, DECISIONS.md updated or explicitly
